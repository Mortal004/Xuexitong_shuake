...
def use_extension(item0,i):
    ...
    while k<=0.7:
        try:
            ...
            size = item0.size
            num1 = pyautogui.locateOnScreen(fr'task\img\img_{i}.png', confidence=0.9-k)
            print(color.blue(color.magenta(f'正在搜索第{i + 1}题')))
            pyautogui.moveTo(num1, duration=0.5)
            pyautogui.dragRel(size['width'], size['height'] + 80, duration=1)
            # print(size['width'], size['height'], i)
            time.sleep(3)
            return size['height'] + 80
        except:
            print(color.yellow('未知错误,再来一次'))
            k+=0.1
            continue

def get_question_date(driver,course_name):
    time.sleep(3)
    driver.switch_to.default_content()
    driver.switch_to.frame('iframe')
    # 判断是否完成任务
    element = driver.find_element(By.XPATH, '//*[@id="ext-gen1049"]/div[2]/div/p/div/div')
    txt = element.get_attribute('aria-label')
    if txt == '任务点已完成':
        pyautogui.scroll(-250)
        print(color.green('测试已完成\n点击下一节'))
        # driver.switch_to.default_content()
        ...
        print(color.yellow("当前页面共有{}题".format(len(questionList0))))
        ...
        for i in range(len(questionList0)):
            ...
            if questionType not in ("单选题", "多选题", "判断题"):
                ...
                print(color.yellow("本题类型为：" + questionType))
                print(color.yellow("跳过该题"))
                # print(-int(round(item0.size['height']*0.66)))
            ...
            if answerList is None or len(answerList) == 0:
                print(color.yellow('未找到该题答案，开始下一题'))
                continue
            driver.switch_to.frame('iframe')
            element = driver.find_element(By.XPATH, '//*[@id="ext-gen1049"]/div[2]/div/p/div/iframe')
            driver.switch_to.frame(element)
...
def __submit(driver,course_name,ans_rate):
    formatted_result="{:.2%}".format(ans_rate)
    print(color.red(f'本次答题率为{formatted_result}'))
    if ans_rate>=0.9:
        # 点击提交
        print(color.yellow('5秒后提交'))
        time.sleep(5)
    ...
    # 下一节
    print(color.green('开始下一节'))
    # driver.find_element(By.XPATH, '//*[@id="prevNextFocusNext"]').click()
    return

def save_score(driver,course_name):
